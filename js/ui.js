...
// ── SÉLECTEUR PLATEFORME RÉUTILISABLE (logos + menu déroulant maison) ──
// Même rendu que le sélecteur plateforme de l'étape 2 du mode Script
// (index.html, #platformPicker, HTML statique historique, non touché ici
// pour ne rien risquer sur ce qui marche déjà), mais générique : accepte
// n'importe quel id, pour les autres endroits qui ont besoin du même choix
// (ex. Storyboard seul, voir js/storyboard-seul.js), sans dupliquer les
// logos ni la mécanique d'ouverture/fermeture.
const _LOGOS_PLATEFORME = {
  'TikTok': '<path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-2.88 2.5 2.89 2.89 0 0 1-2.89-2.89 2.89 2.89 0 0 1 2.89-2.89c.28 0 .54.04.79.1V9.01a6.33 6.33 0 0 0-.79-.05 6.34 6.34 0 0 0-6.34 6.34 6.34 6.34 0 0 0 6.34 6.34 6.34 6.34 0 0 0 6.33-6.34V8.69a8.18 8.18 0 0 0 4.78 1.52V6.76a4.85 4.85 0 0 1-1.01-.07z"/>',
  'Instagram Reels': '<path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>',
  'YouTube Shorts': '<path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>',
  'Facebook': '<path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>'
};
_LOGOS_PLATEFORME['YouTube'] = _LOGOS_PLATEFORME['YouTube Shorts']; // Storyboard seul stocke "YouTube", pas "YouTube Shorts"

function _logoPlateforme(nom) {
  return '<svg class="pf-logo" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">' + (_LOGOS_PLATEFORME[nom] || '') + '</svg>';
}

// `id` : identifiant unique du widget (plusieurs instances possibles sur la
// même page). `valeurActuelle` : plateforme déjà choisie, vide = aucune
// ("Choisir…", comme un <select> pas encore touché). `liste` : plateformes
// proposées, dans l'ordre. `onChangeFn` : nom (chaîne) d'une fonction
// globale appelée avec la valeur choisie, pour la stocker où l'appelant en
// a besoin (state.plateforme, sbSeulPlatform…), sans coupler ce widget à
// une variable d'état précise.
function platformPickerHTML(id, valeurActuelle, liste, onChangeFn) {
  const val = valeurActuelle || '';
  const cb = onChangeFn || '';
  const options = liste.map(v => '<div class="custom-select-option platform-option' + (v === val ? ' selected' : '') + '" data-val="' + v + '" onclick="choisirPlateformeGenerique(\'' + id + '\',\'' + v + '\',\'' + cb + '\')">' + _logoPlateforme(v) + '<span>' + v + '</span></div>').join('');
  return '<div class="custom-select platform-picker" id="' + id + '">'
    + '<button type="button" class="custom-select-trigger' + (val ? '' : ' placeholder') + '" aria-haspopup="listbox" onclick="togglePlatformPickerGenerique(\'' + id + '\')">'
    + '<span class="pf-logo" aria-hidden="true">' + (val ? _logoPlateforme(val) : '') + '</span>'
    + '<span class="cs-label">' + (val || 'Choisir ta plateforme…') + '</span>'
    + '<svg class="cs-chevron" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M6 9l6 6 6-6"/></svg>'
    + '</button>'
    + '<div class="custom-select-panel"><div class="custom-select-list" role="listbox">' + options + '</div></div>'
    + '</div>';
}
function togglePlatformPickerGenerique(id) {
  const wrap = document.getElementById(id);
  if (!wrap) return;
  document.querySelectorAll('.platform-picker.open').forEach(function (w) { if (w.id !== id) w.classList.remove('open'); });
  wrap.classList.toggle('open');
}
function choisirPlateformeGenerique(id, val, onChangeFn) {
  const wrap = document.getElementById(id);
  if (!wrap) return;
  wrap.querySelectorAll('.platform-option').forEach(function (o) { o.classList.toggle('selected', o.dataset.val === val); });
  const trigger = wrap.querySelector('.custom-select-trigger');
  if (trigger) {
    const label = trigger.querySelector('.cs-label');
    const logoWrap = trigger.querySelector('.pf-logo');
    if (label) label.textContent = val;
    if (logoWrap) logoWrap.innerHTML = _logoPlateforme(val);
    trigger.classList.remove('placeholder');
  }
  wrap.classList.remove('open');
  if (onChangeFn && typeof window[onChangeFn] === 'function') window[onChangeFn](val);
}
document.addEventListener('click', function (e) {
  document.querySelectorAll('.platform-picker.open').forEach(function (wrap) {
    if (!wrap.contains(e.target)) wrap.classList.remove('open');
  });
});
document.addEventListener('keydown', function (e) {
  if (e.key !== 'Escape') return;
  document.querySelectorAll('.platform-picker.open').forEach(function (w) { w.classList.remove('open'); });
});
