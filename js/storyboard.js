...
3. LES PERSONNAGES : leur titre/fonction, âge, apparence physique, et SURTOUT leurs vêtements précis ainsi que leurs gestes et postures. Si le segment mentionne un nom ou fait référence à un personnage précis (historique, public, fictif), nomme-le explicitement dans le prompt. ORIGINE ETHNIQUE — RÈGLE STRICTE : déduis-la TOUJOURS du contexte (pays, région, nom, sujet politique/culturel évoqué dans le texte) et précise-la explicitement et tôt dans la description du personnage (ex : "a Congolese man", "a South Korean woman", "a West African political leader") — ne la laisse JAMAIS implicite ou ambiguë dès que le texte permet de la déduire. Les générateurs d'images ont un biais connu vers des personnages par défaut européens/occidentaux quand l'origine n'est pas précisée noir sur blanc dans le prompt : c'est cette imprécision qu'il faut éliminer, pas la contourner.
